...
from multion.config.structured_configs import SimulatorSensorConfig
...
@dataclass
class SemMapPolicyConfig(HabitatBaselinesBaseConfig):
    use_oracle_map: bool = False
    map_size: int = 50
    map_channels: int = 4
    coordinate_min: float = -62.3241
    coordinate_max: float = 90.0399
    #meters_per_pixel: 0.3
    object_padding: int = 0
    object_ind_offset: int = 1
    use_local_map: bool = False
    use_world_loc: bool = True
    map_resolution: float = 0.2
    meters_covered: int = 50
    EXPLORATION_STRATEGY: str = "random"
    EXPLORE_RADIUS: int = 50
    MAX_STEPS_BEFORE_GOAL_SELECTION: int = 50
    global_map_size: int = 250
    USE_LOCAL_MAP_FOR_STUBBORN: bool = True
    local_map_size: int = 15
    collision_threshold: int = 5
    has_rgb: bool = True
    frontier_nearest_select: bool = True
    frontier_nearest_select_dist: int = 15
    frontier_dilation_size: int = 10
...
class PolicyConfig(HabitatBaselinesBaseConfig):
    ...
    obs_transforms: Dict[str, ObsTransformConfig] = field(default_factory=lambda : {
        "center_cropper": CenterCropperConfig(),
        "resize_shortest_edge": ResizeShortestEdgeConfig()
    })
...
class RLConfig(HabitatBaselinesBaseConfig):
    ...
    policy: PolicyConfig = PolicyConfig() #Dict[str, PolicyConfig] = field(
...
@dataclass
class TaskBaselinesConfig(HabitatBaselinesBaseConfig):
    # custom task config 
    eval_non_learning: bool = False
...
cs.store(
    group="habitat_baselines/rl/sem_map_policy", name="sem_map_policy_base", node=SemMapPolicyConfig
)
cs.store(
