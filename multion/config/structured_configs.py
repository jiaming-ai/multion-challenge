...
__all__ = [
    ...
    "MultiObjectGoalSensorConfig",
    "PositionSensorConfig",
    "RotationSensorConfig",
    "ProgressMeasurementConfig",
    "PPLMeasurementConfig",
    "EpisodeLengthMeasurementConfig",
    "SubSuccessMeasurementConfig",
    "DistanceToMultiGoalMeasurementConfig",
    "DistanceToCurrGoalMeasurementConfig",
    "DistanceToCurrGoalRewardMeasurementConfig",
    "MSPLMeasurementConfig",
]
...
@dataclass
class MultiObjectGoalSensorConfig(LabSensorConfig):
    r"""
    For MultiObject Navigation tasks only. Generates a discrete observation containing
    the id of the goal object for the episode.

    :property goal_spec: A string that can take the value TASK_CATEGORY_ID or OBJECT_ID. If the value is TASK_CATEGORY_ID, then the observation will be the id of the `episode.object_category` attribute, if the value is OBJECT_ID, then the observation will be the id of the first goal object.
    :property goal_spec_max_val: If the `goal_spec` is OBJECT_ID, then `goal_spec_max_val` is the total number of different objects that can be goals. Note that this value must be greater than the largest episode goal category id.
    """
    type: str = "MultiObjectGoalSensor"
    goal_spec: str = "CATEGORY_LABEL_TEXT"
    goal_spec_max_val: int = 500
...
@dataclass
class PositionSensorConfig(LabSensorConfig):
    r"""
    For Navigation tasks only. The observation of the PositionSensor are two float values
    corresponding to the vector difference in the horizontal plane between the current position
    and the start position of the robot (in meters).
    """
    type: str = "PositionSensor"

@dataclass
class RotationSensorConfig(LabSensorConfig):
    r"""
    For Navigation tasks only. The observation of the
    `RotationSensor` is a single float value corresponding to
    the angle difference in radians between the current rotation of the robot and the
    start rotation of the robot along the vertical axis.
    """
    type: str = "RotationSensor"
...
@dataclass
class ProgressMeasurementConfig(MeasurementConfig):
    type: str = "Progress"
    
@dataclass
class PPLMeasurementConfig(MeasurementConfig):
    type: str = "PPL"
...
@dataclass
class DistanceToCurrGoalRewardMeasurementConfig(MeasurementConfig):
    type: str = "DistanceToCurrGoalReward"
...
class TaskConfig(HabitatBaseConfig):
    ...
    # Custom reward measures
    reward_measure_multi_goal: str = "distance_to_multi_goal"
    subsuccess_measure: str = "sub_success"
    subsuccess_reward_measure: str = "ppl"
    subsuccess_reward: float = 3.0
    false_found_penalty: bool = False
    false_found_penalty_value: float = 2.5
...
## Custom sensors
cs.store(
    package="habitat.task.lab_sensors.multiobjectgoal_sensor",
    group="habitat/task/lab_sensors",
    name="multiobjectgoal_sensor",
    node=MultiObjectGoalSensorConfig,
)
cs.store(
    package="habitat.task.lab_sensors.position_sensor",
    group="habitat/task/lab_sensors",
    name="position_sensor",
    node=PositionSensorConfig,
)
cs.store(
    package="habitat.task.lab_sensors.rotation_sensor",
    group="habitat/task/lab_sensors",
    name="rotation_sensor",
    node=RotationSensorConfig,
)
...
## Custom measurements
cs.store(
    package="habitat.task.measurements.progress",
    group="habitat/task/measurements",
    name="progress",
    node=ProgressMeasurementConfig,
)
cs.store(
    package="habitat.task.measurements.ppl",
    group="habitat/task/measurements",
    name="ppl",
    node=PPLMeasurementConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.measurements.distance_to_currgoal",
    group="habitat/task/measurements",
    name="distance_to_currgoal",
    node=DistanceToCurrGoalMeasurementConfig,
)
cs.store(
    package="habitat.task.measurements.distance_to_currgoal_reward",
    group="habitat/task/measurements",
    name="distance_to_currgoal_reward",
    node=DistanceToCurrGoalRewardMeasurementConfig,
)
cs.store(
